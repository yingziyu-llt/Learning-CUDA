...
template <typename T> T trace(const std::vector<T> &h_input, size_t rows, size_t cols) {
    ...
    musaMalloc((void **)&d_input, size);
    musaMemcpyAsync(d_input, h_input.data(), size, musaMemcpyHostToDevice);
...
__global__ void flash_fwd_kernel_half(const half *Q, const half *K, const half *V, half *O, int batch_size,
    ...
    __shared__ half v_tile_fp16[Bc][D_HEAD]; // Pass 2 用
    ...
    for (int col_block_start = 0; col_block_start < seq_len_k; col_block_start += Bc) {
        ...
        for (int i = 0; i < ROWS_PER_THREAD; ++i) {
            ...
            for (int k = 0; k < Bc; ++k) {
                ...
                float score = 0.0f;
                for (int d = 0; d < D_HEAD; ++d) {
                    score += (float)q_tile_fp16[q_local_row][d] * (float)k_tile_fp16[k][d];
                }
                score *= scale;
                m_global[i] = fmaxf(m_global[i], score);
            }
    ...
    for (int col_block_start = 0; col_block_start < seq_len_k; col_block_start += Bc) {
        ...
        for (int i = 0; i < ROWS_PER_THREAD; ++i) {
            ...
            for (int k = 0; k < Bc; ++k) {
                ...
                float score = 0.0f;
                for (int d = 0; d < D_HEAD; ++d) score += (float)q_tile_fp16[q_local_row][d] * (float)k_tile_fp16[k][d];
                score *= scale;
                l_global[i] += expf(score - m_global[i]); 
            }
        }
    ...
    for (int col_block_start = 0; col_block_start < seq_len_k; col_block_start += Bc) {
        ...
        for (int i = 0; i < ROWS_PER_THREAD; ++i) {
            ...
            for (int k = 0; k < Bc; ++k) {
                ...
                float p_val = expf(score - m_global[i]); 
                for (int d = 0; d < D_HEAD; ++d) {
                    acc[i][d] += (float)p_val * (float)v_tile_fp16[k][d];
                }
            }
        }
        __syncthreads();
...
void flashAttention(const std::vector<T> &h_q, const std::vector<T> &h_k, const std::vector<T> &h_v,
    ...
    int q_elements = (int)batch_size * target_seq_len * query_heads * head_dim;
    int k_elements = (int)batch_size * src_seq_len * kv_heads * head_dim;
    int v_elements = (int)batch_size * src_seq_len * kv_heads * head_dim;
    int o_elements = q_elements;
    ...
    RUNTIME_CHECK(musaMalloc(&d_k, k_bytes));
    RUNTIME_CHECK(musaMalloc(&d_v, v_bytes));
    RUNTIME_CHECK(musaMalloc(&d_o, o_bytes));

    RUNTIME_CHECK(musaMemcpy(d_q, h_q.data(), q_bytes, musaMemcpyHostToDevice));
    RUNTIME_CHECK(musaMemcpy(d_k, h_k.data(), k_bytes, musaMemcpyHostToDevice));
    RUNTIME_CHECK(musaMemcpy(d_v, h_v.data(), v_bytes, musaMemcpyHostToDevice));
    ...
    RUNTIME_CHECK(musaMemcpy(h_o.data(), d_o, o_bytes, musaMemcpyDeviceToHost));
